...
public:
	virtual TFltV observationEq()	// non-linear observation equation implementation, has dimension MP
		{ TFltV test; return test; };  // dummy implementation
	virtual TFltV transitionEq()	// non-linear transition equation implementation, has dimension DP	
		{ TFltV test; return test; };  // dummy implementation
public:
	ExtendedKalmanFilter() {};
	ExtendedKalmanFilter(int dynamParams, int measureParams, int controlParams);	
	TFltV Predict(TFltV control);
	TFltV Correct(TFltV measurement);
};
#endif