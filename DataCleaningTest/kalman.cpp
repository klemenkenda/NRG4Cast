...
ExtendedKalmanFilter::ExtendedKalmanFilter(int dynamParams, int measureParams, int controlParams) {
	// dimensions
	DP = dynamParams;
	MP = measureParams;
	CP = controlParams;
...
TFltV ExtendedKalmanFilter::Predict(TFltV control) {
	
	// update the state: x'(k) = A * x(k)
	// standard KF: TLinAlg::Multiply(transitionMatrix, statePost, statePre);
	statePre = transitionEq();
...
TFltV ExtendedKalmanFilter::Correct(TFltV measurement) {
	// temp2 = H * P'(k)
	TLinAlg::Multiply(measurementMatrix, errorCovPre, temp2VV);
	...
	// temp2V = z(k) - H*x'(k)
	temp1V.Gen(1, 1);
	// standardKF: TLinAlg::Multiply(measurementMatrix, statePre, temp1V);
	temp1V = observationEq();
	temp2V.Gen(measurement.Len(), measurement.Len());
	TLinAlg::AddVec(-1.0, temp1V, measurement, temp2V);
